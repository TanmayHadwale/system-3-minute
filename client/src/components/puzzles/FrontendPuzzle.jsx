// src/components/puzzles/FrontendPuzzle.jsx
import { useState } from 'react';
import { useGame } from '../../context/GameContext';
import HintButton from '../HintButton';

export default function FrontendPuzzle() {
  const { chain, solveModule, solvedModules, status, clues } = useGame();
  const puzzle = chain?.modules?.frontend;
  ...
  const isRestored = solvedModules.frontend;

  const handleSubmit = (e) => {
    e?.preventDefault();
    if (!answer.trim() || status !== 'playing' || isRestored) return;

    const result = solveModule('frontend', answer);
  ...
  // Build display code — replace ____ with the input value or blank
  const displayCode = puzzle.code.map(line => {
    if (line.includes('____')) {
      return line.replace('____', answer || '____');
    }
    return line;
  });
  ...
  return (
    <div className="animate-fade-in h-full flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="font-mono font-bold text-white text-base tracking-widest">FRONTEND</h2>
        <span className={`badge ${isRestored ? 'badge-restored' : 'badge-warning'}`}>
          {isRestored ? '✓ RESTORED' : 'CORRUPTED'}
        </span>
      </div>

      <div className="text-xs font-mono text-slate-400 border-l-2 border-orange-600 pl-3 py-1">
        {puzzle.objective}
      </div>

      {isRestored ? (
        <div className="glass-panel rounded-lg p-6 neon-border-green animate-scale-in flex flex-col gap-3">
          <div className="font-mono text-green-400 font-bold text-lg text-glow-green">✓ FRONTEND RESTORED</div>
          {puzzle.clueReward && (
          ...
          <div className="text-xs font-mono text-slate-400">Proceed to the LOGIC ENGINE module.</div>
        </div>
      ) : (
        <>
          {/* Clue reminder */}
          ...
          <div className="glass-panel rounded-lg p-5 flex flex-col gap-4 flex-1">
            <div className="font-mono text-sm text-orange-400 font-bold">⚠ CODE REPAIR REQUIRED</div>

            {/* Code block */}
            <div className="code-block">
              {displayCode.map((line, i) => {
                if (line.includes('____') || (answer && line.includes(answer))) {
                  const parts = line.split(answer || '____');
                  return (
                    <div key={i} className="whitespace-pre">
                      <span className="text-slate-400">{parts[0]}</span>
                      <span className="code-blank">{answer || '____'}</span>
                      <span className="text-slate-400">{parts[1]}</span>
                    </div>
                  );
                }
                return <div key={i} className="text-slate-500 whitespace-pre">{line}</div>;
              })}
            </div>

            {/* Hint text */}
            <div className="text-xs font-mono text-slate-500 bg-slate-900/50 rounded p-2 border border-slate-800">
              {puzzle.clueText}
            </div>

            {/* Input */}
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
              <div className="text-xs font-mono text-slate-400">Enter the missing value:</div>
              <input
                type="text"
                value={answer}
              ...
              <div className="flex items-center gap-3">
                ...
                <HintButton moduleName="frontend" />
              </div>
              {feedback && (
